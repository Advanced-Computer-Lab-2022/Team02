...
const ExerciseDetails = ({exercises}) =>{
    ...
    const startQuiz= async() =>{
        const response = await fetch(`/indTrainee/startQuiz`, {
            method: 'POST',
        ...
        if (response.ok){
            window.location.href=`/Quiz?exerciseID=${exercises._id}&courseId=${courseId}`
            alert("Quiz Started")
        }
    }
    const getGrade= async() =>{
        const response = await fetch(`/indTrainee/grade`, {
            method: 'POST',
